...
이 파일은 d:/ClaudeCode/higgsfield-spotlight/backend/favorites_store.py 와
동기 유지 — 변경 시 양쪽 동시 적용. PV 쪽이 canonical.
...
class FavoritesLock:
    ...
    def __exit__(self, exc_type, exc, tb):
        try:
            # 본인이 만든 락 파일만 unlink — 남의 락 침해 방지
            if self._acquired:
                try:
                    self._lock_path.unlink()
                except OSError:
...
def _maybe_migrate_legacy_global() -> int:
    ...
    # 원본 백업 — collision 안전하게 (.migrated, .migrated.1 등)
    backup_path = LEGACY_GLOBAL_FAVS.with_suffix(LEGACY_BACKUP_SUFFIX)
    if backup_path.exists():
        n = 1
        while True:
            candidate = LEGACY_GLOBAL_FAVS.with_suffix(f"{LEGACY_BACKUP_SUFFIX}.{n}")
            if not candidate.exists():
                backup_path = candidate
                break
            n += 1
    try:
        # 원본을 백업으로 이동 (분류된/안된 entry 모두 포함 — 원본 보존)
        import shutil
        shutil.copy2(str(LEGACY_GLOBAL_FAVS), str(backup_path))
    except OSError:
        pass

    # 분류 못 한 entry 는 LEGACY_GLOBAL_FAVS 에 다시 써서 보존.
    # all_favorites() 가 이 파일을 읽으므로 entry 들이 계속 노출됨.
    # (이전엔 원본을 rename 만 해서 unclassified entry 가 사라지는 데이터 손실 버그)
    try:
        if unclassified:
            LEGACY_GLOBAL_FAVS.parent.mkdir(parents=True, exist_ok=True)
            tmp = LEGACY_GLOBAL_FAVS.with_suffix(".json.tmp")
            tmp.write_text(json.dumps(unclassified, ensure_ascii=False, indent=2),
                           encoding="utf-8")
            os.replace(tmp, LEGACY_GLOBAL_FAVS)
        else:
            # 전부 분류 성공 → 원본 제거 (이미 백업됨)
            LEGACY_GLOBAL_FAVS.unlink(missing_ok=True)
    except OSError:
        pass
