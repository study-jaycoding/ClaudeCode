...
// =====================================================================
import {
    viewerTrackList, viewerAddBtn, viewerClearBtn, viewerTrackTotal,
...
function _renderPickerCard(node) {
    ...
    el.addEventListener("click", () => {
        const ok = addPathToTrack(node.path);
        if (ok) {
            el.classList.add("vp-added-flash");
            setTimeout(() => el.classList.remove("vp-added-flash"), 400);
            viewerPickerAdded.textContent = `${_track.length}개 슬롯 / 총 ${_totalLayerCount()}개 레이어`;
        }
    });
...
function _totalLayerCount() { return _track.reduce((s, sl) => s + (sl.layers?.length || 0), 0); }

// === 이벤트 ===

viewerAddBtn?.addEventListener("click", _pickerOpen);
...
viewerPickerClose?.addEventListener("click", _pickerClose);
...
viewerPicker?.addEventListener("click", (e) => { if (e.target === viewerPicker) _pickerClose(); });
...
document.addEventListener("keydown", (e) => {
    ...
    if (_isPickerOpen()) {
        if (e.key === "Escape") { _pickerClose(); e.preventDefault(); }
        return;
    }
