...
import {
    ...
    promptRowEl, promptInput,
    modelDropdown, ratioDropdown, projectDropdown,
    tagFilterBadge,
} from "./dom.js";
import { cache, pickerState } from "./state.js";
import { escapeHtml } from "./utils.js";
import { getBackslashQueryInfo, stripBackslashQuery } from "./prompt.js";
import { registerCloser } from "./dropdowns.js";

function getAllTags() {
    const set = new Set();
    cache.favorites.forEach((f) => Array.isArray(f.tags) && f.tags.forEach((t) => set.add(t)));
    return [...set].sort();
}
...
export function renderTagList() {
    ...
    const filter = bs ? bs.query.toLowerCase() : "";
    ...
    pickerState.filteredTags.forEach((tag, i) => {
        const count = cache.favorites.filter((f) => Array.isArray(f.tags) && f.tags.includes(tag)).length;
