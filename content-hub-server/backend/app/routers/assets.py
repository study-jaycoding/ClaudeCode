...
def asset_meta(request: Request, project: str = Query(...)):
    ...
    _require_project(project, actor_id(request))
...
def add_comment(body: CommentAddIn, request: Request):
    _require_project(body.project, actor_id(request))
...
def asset_set_source(body: AssetSourceIn, request: Request):
    owner = actor_id(request)
    _require_project(body.project, owner)
    # 에셋 메타는 계정별 개인화 — 내(actor_id) 설정만 만들고 바꾼다(남의 것과 안 섞임).
    repo.set_asset_source(body.project, body.path, body.name, body.is_source, owner)
    return {"ok": True}
...
def asset_set_tags(body: AssetTagsIn, request: Request):
    owner = actor_id(request)
    _require_project(body.project, owner)
    repo.set_asset_tags(body.project, body.path, body.tags, owner)
    return {"ok": True}
...
def asset_set_comment(body: AssetCommentIn, request: Request):
    owner = actor_id(request)
    _require_project(body.project, owner)
    repo.set_asset_comment(body.project, body.path, body.comment, owner)
    return {"ok": True}
...
def asset_set_color(body: AssetColorIn, request: Request):
    owner = actor_id(request)
    _require_project(body.project, owner)
    repo.set_asset_color(body.project, body.path, body.color, owner)
    return {"ok": True}
