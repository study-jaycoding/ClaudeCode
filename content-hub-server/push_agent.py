...
def push_once(server: str, token: str, cli: str, size: int) -> None:
    ...
    if status != 200 or not isinstance(body, dict):
        sys.exit(f"[오류] 적재 실패(status={status}): {body}")
    print(
...
def main() -> None:
    ...
    ap.add_argument("--email", required=True, help="내 허브 로그인 이메일")
    ...
    if args.token:
        token = args.token
        print(f"[토큰] 전달된 세션 토큰 사용({args.email})")
    else:
    ...
    def cycle() -> None:
        ...
        # ② 내가 로컬에서 만든(또는 방금 실행한) 결과물을 서버로 push
